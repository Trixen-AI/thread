...
/**
 * The RPC MESH reads Robinhood Chain through: the endpoint the chain itself
 * publishes, and the same one handed to a wallet by `addChainParams`. One
 * source for both, so what MESH reads and what a wallet is told to use can
 * never drift apart.
 */
export const robinhoodRpcUrl = () => robinhoodChain.rpcUrls.default.http[0]
